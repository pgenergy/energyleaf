...
import { updateSimulationEvSettingsAction } from "@/server/actions/simulations";
...
export default function EvForm(props: Props) {
	const defaultValues: EvSettingsInput = {
		chargingSpeed: props.initialValues.chargingSpeed,
		evCapacityKwh: props.initialValues.evCapacityKwh,
		dailyDrivingDistanceKm: props.initialValues.dailyDrivingDistanceKm,
		avgConsumptionPer100Km: props.initialValues.avgConsumptionPer100Km,
		defaultSchedule:
			props.initialValues.defaultSchedule.length > 0
				? props.initialValues.defaultSchedule
				: [{ start: "22:00", end: "06:00" }],
		weekdaySchedules: props.initialValues.weekdaySchedules,
	};
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const res = await updateSimulationEvSettingsAction(value as z.infer<typeof evSettingsSchema>);
			if (!res.success) {
