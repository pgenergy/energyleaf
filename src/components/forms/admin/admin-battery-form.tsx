...
import { adminUpdateSimulationBatterySettingsAction } from "@/server/actions/admin";
...
export default function AdminBatteryForm({ userId, initialValues }: Props) {
	const defaultValues: z.input<typeof batterySettingsSchema> = {
		capacityKwh: initialValues.capacityKwh,
		maxPowerKw: initialValues.maxPowerKw,
	};
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const res = await adminUpdateSimulationBatterySettingsAction(
				userId,
				value as z.infer<typeof batterySettingsSchema>,
			);
			if (!res.success) {
