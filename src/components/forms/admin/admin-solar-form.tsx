...
import { adminUpdateSimulationSolarSettingsAction } from "@/server/actions/admin";
...
export default function AdminSolarForm({ userId, initialValues }: Props) {
	const defaultValues: z.input<typeof solarSettingsSchema> = {
		peakPower: initialValues.peakPower,
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const res = await adminUpdateSimulationSolarSettingsAction(
				userId,
				value as z.infer<typeof solarSettingsSchema>,
			);
			if (!res.success) {
