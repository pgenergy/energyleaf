...
import { getSensorsPage } from "@/server/queries/sensor";
...
export async function SensorsOverview({ page, pageSize, query }: Props) {
	const { sensors, total } = await getSensorsPage({ page, pageSize, query });
	...
	const buildHref = (targetPage: number) => {
		...
		return `/admin/sensors?${params.toString()}`;
	};
	...
	if (total === 0 && hasQuery) {
		return (
			<div className="col-span-1 flex flex-col items-center gap-4 md:col-span-2">
				<p className="text-center font-mono font-semibold">Keine Sensoren für diese Suche gefunden.</p>
			</div>
		);
	}

	if (sensors.length === 0 && total > 0) {
		return (
			<div className="col-span-1 flex flex-col items-center gap-4 md:col-span-2">
				<p className="text-center font-mono font-semibold">Diese Seite enthält keine Sensoren.</p>
		...
		<>
			...
			<div className="col-span-1 flex flex-col items-center gap-2 md:col-span-2">
				<p className="text-muted-foreground text-sm">
					{startIdx}–{endIdx} von {total} Sensoren
				</p>
				{pageCount > 1 && (
					<Pagination>
						<PaginationContent>
