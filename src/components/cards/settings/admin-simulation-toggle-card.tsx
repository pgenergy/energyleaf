...
import { adminToggleSimulationEnabledAction } from "@/server/actions/admin";
...
export default function AdminSimulationToggleCard({ userId, simulationType, enabled, configured }: Props) {
	const [isPending, startTransition] = useTransition();

	const handleToggle = (checked: boolean) => {
		startTransition(async () => {
			const toastId = toast.loading("Speichern...", { duration: Infinity });
			const res = await adminToggleSimulationEnabledAction(userId, simulationType, checked);
			if (!res.success) {
			...
			<CardHeader>
				<div className="flex flex-row items-center justify-between gap-4">
					<div className="flex flex-col gap-1">
						...
						<CardDescription>
							{configured
								? `Aktivieren oder deaktivieren Sie die ${displayName}-Simulation für diesen Benutzer.`
