...
import { UserHouseholdCard } from "@/components/cards/admin/user-edit-cards";
...
export const metadata: Metadata = {
	title: "Haushalt bearbeiten - Admin - Energyleaf",
};
...
async function UserEditHeader({ userId }: { userId: string }) {
	...
	return (
		<div className="flex flex-col gap-1">
			<h1 className="text-xl font-semibold">{displayName} - Haushalt</h1>
			<p className="text-muted-foreground text-sm">{user.email}</p>
		</div>
	);
}

export default async function UserEditHouseholdPage(props: Props) {
		...
		<>
			<Breadcrumb className="mb-4">
				<BreadcrumbList>
					...
					<BreadcrumbSeparator />
					<BreadcrumbItem>
						<BreadcrumbPage>Haushalt</BreadcrumbPage>
					</BreadcrumbItem>
				</BreadcrumbList>
			</Breadcrumb>
