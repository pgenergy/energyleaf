...
export const metadata: Metadata = {
	title: "Tarif bearbeiten - Admin - Energyleaf",
};
...
async function UserEditHeader({ userId }: { userId: string }) {
	...
	return (
		<div className="flex flex-col gap-1">
			<h1 className="text-xl font-semibold">{displayName} - Tarif</h1>
			<p className="text-muted-foreground text-sm">{user.email}</p>
		</div>
	);
}

export default async function UserEditTariffPage(props: Props) {
		...
		<>
			<Breadcrumb className="mb-4">
				<BreadcrumbList>
					...
					<BreadcrumbSeparator />
					<BreadcrumbItem>
						<BreadcrumbPage>Tarif</BreadcrumbPage>
					</BreadcrumbItem>
				</BreadcrumbList>
			</Breadcrumb>
