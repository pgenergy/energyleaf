...
import {
	UserBatterySimulationCard,
	UserEvSimulationCard,
	UserHeatPumpSimulationCard,
	UserSolarSimulationCard,
	UserTouTariffSimulationCard,
...
export const metadata: Metadata = {
	title: "Simulationen bearbeiten - Admin - Energyleaf",
};
...
async function UserEditHeader({ userId }: { userId: string }) {
	...
	return (
		<div className="flex flex-col gap-1">
			<h1 className="text-xl font-semibold">{displayName} - Simulationen</h1>
			<p className="text-muted-foreground text-sm">{user.email}</p>
		</div>
	);
}

export default async function UserEditSimulationsPage(props: Props) {
		...
		<>
			<Breadcrumb className="mb-4">
				<BreadcrumbList>
					...
					<BreadcrumbSeparator />
					<BreadcrumbItem>
						<BreadcrumbPage>Simulationen</BreadcrumbPage>
					</BreadcrumbItem>
				</BreadcrumbList>
			</Breadcrumb>
