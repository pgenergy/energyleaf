...
import { UserContactCard, UserNameCard } from "@/components/cards/admin/user-edit-cards";
...
export const metadata: Metadata = {
	title: "Profil bearbeiten - Admin - Energyleaf",
};
...
async function UserEditHeader({ userId }: { userId: string }) {
	...
	return (
		<div className="flex flex-col gap-1">
			<h1 className="text-xl font-semibold">{displayName} - Profil</h1>
			<p className="text-muted-foreground text-sm">{user.email}</p>
		</div>
	);
}

export default async function UserEditProfilePage(props: Props) {
